...
function speakAsync(text, volume, voice) {
  return new Promise(res => {
    if (!window.speechSynthesis) { devLog(T('(브라우저가 TTS 를 지원하지 않음)'), 'warn'); return res(); }
...
function buildInterpreterApi(interp, scope) {
  ...
  setAsync('simPlayMotion', (name, cycle, cb) => {
    ...
    devLog('▶ ' + n);
    Pibo.playMotion(n, Number(cycle) || 1).then(() => cb()).catch(err => { devLog(T('모션 오류') + ': ' + err, 'err'); cb(); });
  });
}
...
function devRun() {
  ...
  try {
    ...
    devLog(T('코드 생성 실패') + ': ' + e.message, 'err'); return;
  }
  if (!code.trim()) {
    const has = devWorkspace.getAllBlocks(false).length > 0;
    devLog(has ? T("'시작' 블록에 연결된 블록이 없습니다. 시작 아래에 붙여주세요.")
               : T('블록이 없습니다.'), 'warn');
    return;
  }
  ...
  try {
    ...
    devLog(T('실행 준비 실패') + ': ' + e.message, 'err'); return;
  }
