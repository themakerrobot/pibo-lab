...
const gFmtTime = sec => {
  ...
  return m > 0 ? m + PIBO_T('분') + s2.toFixed(1) + PIBO_T('초') : s2.toFixed(1) + PIBO_T('초');
};
...
(function () {
  ...
  sp.innerHTML = '<span class="lb">' + PIBO_T('시간') + '</span><b id="gTime">0:00</b>';
  hud.appendChild(sp);
  setInterval(() => {
...
function gSpeakAsync(text) {
  return new Promise(res => {
    if (!window.speechSynthesis) { gLog(PIBO_T('(브라우저가 TTS 를 지원하지 않음)'), 'warn'); return res(); }
...
function buildGameApi(interp, scope) {
  ...
  setAsync('gMove', (n, cb) => {
    ...
    const next = () => {
      ...
      if (gWallAhead(dir)) { Game.say(PIBO_T('벽에 막혔어요')); return cb(); }
      i++;
  ...
  set('gLife', n => {
    Game.addLife(nat(n));
    if (Game.lives <= 0) { Game.say(PIBO_T('게임 오버')); gLog(PIBO_T('게임 오버'), 'warn'); gameStop(); }
  });
  ...
  set('gOver', r => {
    ...
    Game.say(win ? PIBO_T('성공!') + ' (' + t + ')' : PIBO_T('실패…'));
    gLog((win ? PIBO_T('게임 성공') : PIBO_T('게임 실패')) + ' — ' + PIBO_T('걸린 시간') + ' ' + t, win ? '' : 'warn');
    gameStop();
  });
...
function gameRun() {
  ...
  catch (e) { gLog(PIBO_T('코드 생성 실패') + ': ' + e.message, 'err'); return; }
  if (!code.trim()) {
    const has = gameWorkspace.getAllBlocks(false).length > 0;
    gLog(has ? PIBO_T("'게임 시작하면' 블록에 연결해 주세요.") : PIBO_T('블록이 없습니다.'), 'warn');
    return;
  }
  ...
  catch (e) { gLog(PIBO_T('실행 준비 실패') + ': ' + e.message, 'err'); return; }
  ...
  (function loop() {
    ...
    } catch (e) { gLog(PIBO_T('실행 오류') + ': ' + e.message, 'err'); gameStop(); return; }
    if (!more) { gLog(PIBO_T('실행 끝')); gameStop(); return; }
