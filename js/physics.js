...
window.PHYS = {
  ...
  build(){
    if(!robotRoot){alert(T('로봇을 먼저 로드하세요'));return false;}
  ...
  enable(){
    if(!this.ready){alert(T('물리 엔진 로딩 중입니다'));return;}
    if(this.on) return;
  ...
  step(now){
    ...
    if(fell!==this.fallen){
      ...
      uiText('physState',fell?T('넘어짐'):'ON');
    }
...
window.addEventListener('rapier-fail',()=>{
  uiText('physEngine',T('실패'));
  uiSet('physBtn','title',T('Rapier 로드 실패 — 콘솔(F12) 확인'));
});
