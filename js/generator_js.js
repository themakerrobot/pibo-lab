...
(function () {
  ...
  G['motion_set_speed'] = b =>
    `simNote('set_speed(' + ${b.getFieldValue('no')} + ', ' + ${val(b, 'val', '0')} + ') — ${T('시뮬 미반영')}');\n`;
  G['motion_set_acceleration'] = b =>
    `simNote('set_acceleration(' + ${b.getFieldValue('no')} + ', ' + ${val(b, 'val', '0')} + ') — ${T('시뮬 미반영')}');\n`;
  ...
  Blockly.Blocks && Object.keys(Blockly.Blocks).forEach(t => {
    if (G[t]) return;
    G[t] = function (block) {
      const unsupported = `// ${t}: ${T('시뮬 미지원')}\n`;
      return block.outputConnection ? ['null', ORD_ATOMIC] : `simNote('${t} — ${T('시뮬 미지원, 건너뜀')}');\n`;
    };
  });
})();
