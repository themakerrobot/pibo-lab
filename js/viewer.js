...
async function autoLoadFromData(){
  ...
  const fail=(msg)=>{ spin.style.display='none'; title.textContent=T('자동 로드 실패');
    ptext.innerHTML=msg+'<br><span style="color:#B0B7BE">'+T('data/ 경로·네트워크 확인 후 새로고침하세요')+'</span>';
  ...
  title.textContent=T('data/ 에서 불러오는 중...');
  ptext.textContent=T('URDF 가져오는 중...'); pbar.style.width='0%';
  ...
  if(!urdfTxt){ fail(T('URDF를 찾지 못했습니다')+' ('+URDF_NAMES.join(', ')+')'); return; }

  let links;
  try{ links=parseURDF(urdfTxt).links; }catch(e){ fail(T('URDF 파싱 오류')+': '+e.message); return; }
...
async function loadRobot(){
  ...
  try{
    ...
    document.getElementById('infoRows').innerHTML=
      [[T('링크'),Object.keys(links).length],[T('관절'),Object.keys(joints).length],[T('가동'),mv],[T('메시'),allMeshes.length]]
      .map(([k,v])=>`<div class="info-row"><span>${k}</span><span class="info-val">${v}</span></div>`).join('');
    const b=document.getElementById('statusBadge');b.textContent=T('완료');b.className='badge ok';
...
const ACCESSORY = { head_top: T('머리 장식') };
const PART_LABEL = {
  base_link:T('몸통'), head_pan_link:T('목'), head_link:T('머리'),
  shoulder_l_link:T('왼쪽 어깨'), arm_l_link:T('왼팔'),
  shoulder_r_link:T('오른쪽 어깨'), arm_r_link:T('오른팔'),
  leg_l_link:T('왼쪽 다리'), foot_l_link:T('왼발'),
  leg_r_link:T('오른쪽 다리'), foot_r_link:T('오른발'),
};
