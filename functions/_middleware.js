export async function onRequest(context) {
  ...
  // 변수 미설정 시 잠금 해제 (정식 오픈 시 Secret만 지우면 그대로 공개)
  if (!PASS) return next();
...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  .bot .body { fill: var(--brand); }
  .bot .face { fill: #0F1B22; }
  .bot .eye  { fill: var(--brand); }
  .badge {
  ...
  @media (prefers-color-scheme: dark) {
    ...
    body { background:
      radial-gradient(900px 500px at 15% -10%, rgba(0,190,220,.16), transparent 60%),
      #0B1419; }
    .bot .face { fill: #0B1419; }
    .foot { color:#6E838E; }
  }
</style>
</head>
